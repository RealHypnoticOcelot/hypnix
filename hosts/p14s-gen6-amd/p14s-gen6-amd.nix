...
}:
{
  boot.kernelParams = [
    ...
    # Fixes audio randomly cutting out briefly by using a software implementation of IOMMU(memory management unit) instead of
    # the available hardware implementation. Prevents PCI passthrough for VMs, if that's something you're into!
  ];
